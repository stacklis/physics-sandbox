> Archived 2026-05-13: 3D mode shipped in v1.0.0 (commit ebb8e3b). This plan is preserved for context.
...
async function loadRapier() {
  if (!_RAPIER_PROMISE) {
    _RAPIER_PROMISE = (async () => {
      // Bare specifier — resolved by importmap in browser (esm.sh CDN) and
      // by node_modules in Node tests (devDependency installed in Step 4).
